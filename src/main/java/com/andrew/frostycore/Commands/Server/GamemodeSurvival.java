        ...
        if (!cooldown.asMap().containsKey(player.getUniqueId())) {
            ...
            if (args.length != 0) {
                ...
                if (player != target) {
                    ...
                    player.sendMessage(ChatColorUtil.colorize(ServerMessage.getServerPrefix() + " &aYou have set " + target.getName() + "'s gamemode to survival."));
                    target.sendMessage(ChatColorUtil.colorize(ServerMessage.getServerPrefix() + " &a" + player.getName() + " has set your gamemode to survival."));
                    return;
                }
            }
            player.setGameMode(org.bukkit.GameMode.SURVIVAL);
            player.sendMessage(ChatColorUtil.colorize(ServerMessage.getServerPrefix() + " &aYou have set your gamemode to survival."));
        } else {
